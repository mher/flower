...
class TestHumanize(unittest.TestCase):
    ...
    def test_time(self):
        ...
        self.assertEqual(
            format_time(1343911558.305793, utc),
            humanize(1343911558.305793, type="time"),
        )

    def test_natural_time(self):
        self.assertEqual(
            humanize(time.time() - 1, type="natural-time-utc"), "a second ago"
        )
        self.assertEqual(humanize(time.time() - 1, type="natural-time"), "a second ago")

    def test_strings(self):
        self.assertEqual("Max tasks per child", humanize("max_tasks_per_child"))
