...
class TestFlowerCommand(AsyncHTTPTestCase):
    ...
    def test_task_runtime_metric_buckets_read_from_env(self):
        with patch.dict(os.environ, {"FLOWER_TASK_RUNTIME_METRIC_BUCKETS": "2,5,inf"}):
            apply_env_options()
            self.assertEqual(
                [2.0, 5.0, float("inf")], options.task_runtime_metric_buckets
            )
...
class TestPrintBanner(AsyncHTTPTestCase):
    def test_print_banner(self):
        ...
        with self.assertLogs("", level="INFO") as cm:
            ...
            self.assertTrue(
                "INFO:flower.command:Visit me at http://0.0.0.0:5555" in cm.output
            )
            self.assertTrue(
    ...
    def test_print_banner_with_ssl(self):
        ...
        with self.assertLogs("", level="INFO") as cm:
            ...
            self.assertTrue(
                "INFO:flower.command:Visit me at https://0.0.0.0:5555" in cm.output
            )
            self.assertTrue(
                "INFO:flower.command:Broker: amqp://guest:**@localhost:5672//"
                in cm.output
    ...
    def test_print_banner_unix_socket(self):
        ...
        with (
            ...
            self.mock_option("unix_socket", "foo"),
        ):
            print_banner(celery_app, True)

            self.assertTrue(
                "INFO:flower.command:Visit me via unix socket file: foo" in cm.output
            )
...
class TestConfOption(AsyncHTTPTestCase):
    def test_error_conf(self):
        with self.mock_option("conf", None):
            self.assertRaises(IOError, apply_options, "flower", argv=["--conf=foo"])
            self.assertRaises(
                IOError, apply_options, "flower", argv=["--conf=/tmp/flower/foo"]
            )
