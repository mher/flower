...
class TasksTest(AsyncHTTPTestCase):
    ...
    def test_succeeded_task(self):
        ...
        r = self.get(
            "/tasks/datatable?" + "&".join(map(lambda x: "%s=%s" % x, params.items()))
        )
    ...
    def test_sort_runtime(self):
        ...
        events += task_succeeded_events(
            worker="worker1", name="task1", id="2", runtime=10.0
        )
        events += task_succeeded_events(
            worker="worker1", name="task1", id="4", runtime=10000000.0
        )
        events += task_succeeded_events(
            worker="worker1", name="task1", id="3", runtime=20.0
        )
        events += task_succeeded_events(
            worker="worker1", name="task1", id="1", runtime=2.0
        )
        for i, e in enumerate(events):
            e["clock"] = i
            e["local_received"] = time.time()
    ...
    def test_sort_incomparable(self):
        ...
        events += task_succeeded_events(worker="worker1", name="task1", id="123")
        events += task_succeeded_events(
            worker="worker1", name="task1", id="456", runtime=None
        )
        for i, e in enumerate(events):
            e["clock"] = i
            e["local_received"] = time.time()
