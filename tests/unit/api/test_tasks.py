...
class ApplyTests(BaseApiTestCase):
    def test_apply(self):
        result = "result"
        with patch(
            "celery.result.AsyncResult.state", new_callable=PropertyMock
        ) as mock_state:
            with patch(
                "celery.result.AsyncResult.result", new_callable=PropertyMock
            ) as mock_result:
...
class AsyncApplyTests(BaseApiTestCase):
    ...
    def test_async_apply_eta(self):
        ...
        self.assertEqual(200, r.code)
        task.apply_async.assert_called_once_with(args=[], kwargs={}, eta=tomorrow)

    def test_async_apply_countdown(self):
        task = self._app.capp.tasks["foo"] = Mock()
        task.apply_async = Mock(return_value=AsyncResult(123))
        r = self.post("/api/task/async-apply/foo", body='{"countdown": "3"}')

        self.assertEqual(200, r.code)
        task.apply_async.assert_called_once_with(args=[], kwargs={}, countdown=3)

    def test_async_apply_expires(self):
        task = self._app.capp.tasks["foo"] = Mock()
        task.apply_async = Mock(return_value=AsyncResult(123))
        r = self.post("/api/task/async-apply/foo", body='{"expires": "60"}')

        self.assertEqual(200, r.code)
        task.apply_async.assert_called_once_with(args=[], kwargs={}, expires=60)
    ...
    def test_async_apply_expires_datetime(self):
        ...
        self.assertEqual(200, r.code)
        task.apply_async.assert_called_once_with(args=[], kwargs={}, expires=tomorrow)
...
class TaskTests(BaseApiTestCase):
    ...
    def test_tasks_pagination(self):
        ...
        events += task_succeeded_events(worker="worker1", name="task2", id="456")
        events += task_succeeded_events(worker="worker1", name="task3", id="789")
        events += task_succeeded_events(worker="worker1", name="task4", id="666")
        ...
        r = self.get(
            "/api/tasks?" + "&".join(map(lambda x: "%s=%s" % x, params.items()))
        )
