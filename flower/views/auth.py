...
def authenticate(pattern, email):
    return True # disable email validation as we are using OAuth providers to authenticate users
    if '|' in pattern:
...
class GenericOAuthLoginHandler(BaseHandler, tornado.auth.OAuth2Mixin):
    _OAUTH_NO_CALLBACKS = False
    _OAUTH_SETTINGS_KEY = 'oauth'
    ...
    @property
    def base_url(self):
        return os.environ.get('FLOWER_OAUTH2_GENERIC_BASE_URL')
    ...
    async def ensure_openid_config(self):
        if not self._openid_config:
            url = f"{self.base_url}/.well-known/openid-configuration"
            response = await self.get_auth_http_client().fetch(url)
            if response.error:
                raise tornado.auth.AuthError(f"Failed to fetch OpenID configuration: {response}")
            self._openid_config = json.loads(response.body.decode('utf-8'))
        return self._openid_config

    async def get_access_token(self, redirect_uri, code):
        await self.ensure_openid_config()
        body = urlencode({
            "redirect_uri": redirect_uri,
            "code": code,
            "client_id": self.settings[self._OAUTH_SETTINGS_KEY]['key'],
