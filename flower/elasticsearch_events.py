...
ES_CLIENT = Elasticsearch([ELASTICSEARCH_URL], connection_class=RequestsHttpConnection)
...
def send_to_elastic_search(state, event):
    ...
    if active_index_name != get_index_name(current_date):
        try:
            ...
            INDICES_CLIENT.put_mapping(
                doc_type="task", body=body, index=active_index_name
            )
        except TransportError:
            logger.warning(
                "Issue creating or putting alias or mapping: %s", traceback.format_exc()
            )
        else:
            try:
                deleted = delete_old_elasticsearch_indices(
                    current_date, ES_DAY_RETENTION
                )
                logger.info(
                    "Deleted the following older indices from day retention: "
                    "%s, "
                    "indices: %s",
                    ES_DAY_RETENTION,
                    deleted,
    ...
    doc_body = {
        ...
        "received_time": datetime.utcfromtimestamp(received_time).replace(
            tzinfo=pytz.utc
        )
        if received_time
        else None,
        "retried": task.retried,
        "retried_time": datetime.utcfromtimestamp(task.retried).replace(tzinfo=pytz.utc)
        if task.retried
        else None,
        "started": start_time,
        "started_time": datetime.utcfromtimestamp(start_time).replace(tzinfo=pytz.utc)
        if start_time
        else None,
        ...
        "revoked_time": datetime.utcfromtimestamp(task.revoked).replace(tzinfo=pytz.utc)
        if task.revoked
        else None,
        "failed": task.failed,
        "failed_time": datetime.utcfromtimestamp(task.failed).replace(tzinfo=pytz.utc)
        if task.failed
        else None,
        ...
        "timestamp_time": datetime.utcfromtimestamp(task.timestamp).replace(
            tzinfo=pytz.utc
        )
        if task.timestamp
        else None,
...
def delete_old_elasticsearch_indices(current_date, day_cut_off):
    ...
    if indices_return:
        return {
            ...
            "response": ES_CLIENT.indices.delete(index=",".join(indices_return)),
        }
    return None
    ...
    def __init__(
        ...
        **kwargs,
        ...
        try:
            ...
            logger.warning(
                "Elastic search put mapping error (may be bad)", exc_info=True
            )
        try:
            if INDICES_CLIENT.exists(
                index=get_index_name(datetime.now(tz=pytz.utc).date())
            ):
                INDICES_CLIENT.put_alias("task-*", "task")
        except TransportError:
