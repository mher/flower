...
def flower(ctx, tornado_argv):
    ...
    setup_logging()
    app = ctx.obj.app
    # This currently does not work because this is the wrong conf object/not populated
    # fully. But, the workaround would be to set the TZ env var when starting the
    # process.
    set_os_env_tz_from_conf = getattr(
        app.conf, "set_os_environment_timezone_from_conf", False
    )
    if set_os_env_tz_from_conf:
