...
RUN apt-get update \
  && apt-get install -y --no-install-recommends \
     curl ca-certificates tzdata && update-ca-certificates \
...
RUN /root/.local/bin/uv pip install --system --no-cache-dir /tmp/frontend-wheels/*.whl \
    && /root/.local/bin/uv pip install --system --no-cache-dir /flower
